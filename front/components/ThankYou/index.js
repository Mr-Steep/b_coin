...
export function ThankYou({getMultiplier, handleSetActive, getConfirmationComplete, setConfirmationComplete, getIsUserUseMultiplayer, getGlobalMultiplayer}) {
    ...
    return (
        <Modal>
                <div className="fixed top-[50%] rounded-md left-[50%] translate-x-[-50%] translate-y-[-50%] flex flex-col justify-between items-center bg-whiteColor sm:max-w-[350px] max-w-[544px] px-[70px] py-[50px] z-10">
                    <Image src={cross} className="absolute z-10 top-[22px] sx:right-[30px] right-[22px] cursor-pointer"
                           onClick={() => close()}
                           alt={cross} />
                    <div className="flex flex-col justify-between items-center gap-[30px] text-blackColor">
                        ...
                        {!getIsUserUseMultiplayer ?
                            <Link href="/reward"
                                  className="group mx-auto flex gap-[13px] bg-blackColor items-center justify-between rounded-md md:min-w-[293px] max-w-[293px] max-h-[60px] w-full px-[17px] py-[17px]">
                        <span className="text-lg text-whiteColor">
