...
export function Confirmation({getTransactionComplete, setConfirmationComplete, setTransactionComplete, hash, countTokens}) {
    ...
    return (
        <Modal>
            <div className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] flex flex-col justify-between items-center bg-whiteColor sx:max-w-[360px] sm:max-w-[400px] max-w-[485px] w-full rounded-md py-[50px] z-10">
                ...
                <div className="flex flex-col justify-between items-center gap-[30px]">
                    <p className="text-blackColor text-3xl font-medium text-center">Confirm your email</p>
                    <p className="text-blackColor text-base font-normal text-center max-w-[320px]">Enter your email below to receive all details about this transaction.</p>
                    ...
                    + (emailError || checkboxError ? "bg-errorColor" : "bg-gradient-to-r from-[#33EFF1] to-[#198498]")
                        ...
                        <input name="email" type="email"
                               placeholder="Enter your email" className="w-full outline-0 rounded-[6px] text-blackColor px-[22px] py-4 bg-whiteColor" />
                    </div>
                    ...
                    <div className="flex flex-col justify-between items-center gap-[10px] max-w-[320px] w-full">
                        <div className="flex justify-between items-start bg-whiteColor">
                            ...
                            + (isChecked ? "before:border-blackColor after:content-[''] after:absolute after:left-[7px] after:top-[3px] after:rotate-45 after:h-[11px] after:w-[6px] after:border-b-[2px] after:border-blackColor after:border-r-[2px] " : '') }>Read and accept
                               <Link className="font-bold text-blackColor contents" target="_blank" href="https://bnxt.network/terms-conditions/"> Terms of Use </Link>and <Link className="contents font-bold text-blackColor" target="_blank" href="https://bnxt.network/privacy-policy/">Privacy Policy</Link></span>
                        </div>
                        <div className="flex justify-between items-start bg-whiteColor">
                            ...
                            <span className={"flex text-sm font-normal leading-[18px] relative z-0 text-[#414042] before:content-[''] before:inline-block before:w-[20px] before:h-[20px] before:border before:border-solid before:border-[#D0D5DD] before:rounded-[6px] before:mr-[12px] before:min-w-[20px] "
                            + (isCheckedSubmit ? "before:border-blackColor after:content-[''] after:absolute after:left-[7px] after:top-[3px] after:rotate-45 after:h-[11px] after:w-[6px] after:border-b-[2px] after:border-blackColor after:border-r-[2px] " : '') }>Submit the
                                <Link className="font-bold text-blackColor contents" target="_blank" href=""> Buyer's Statement </Link>as my official declaration</span>
                        </div>
                    </div>
                    ...
                    <Link href=""
                          onClick={confirm}
                          className={"group mx-auto flex gap-[20px] bg-blackColor items-center justify-between rounded-md sm:w-[320px] max-h-[60px] w-full px-[17px] py-[17px] " + (isLoading && " opacity-30 pointer-events-none ")}>
                        <span className="text-whiteColor text-lg flex justify-center gap-[10px] items-center">
                            Confirm
                            {isLoading &&
